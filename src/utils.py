...
class BestKeeper(object):
    ...
    def __init__(self, hparams):
        ...
        if hparams.dataset == 'mnist':
            self.z_batch_val_best = np.zeros((hparams.batch_size, hparams.n_z))
        elif hparams.dataset == 'celebA':
            self.z_batch_val_best = np.zeros((hparams.batch_size, hparams.z_dim))
    ...
    def get_best(self):
        return self.x_hat_batch_val_best,self.z_batch_val_best,self.losses_val_best
...
def get_measurement_loss(x_hat, A, y):
    ...
    if A is None:
        ...
        y_hat = np.absolute(np.matmul(x_hat, A))
...
def get_optimal_x_batch(x, xneg, xstar, batch_size):
    """Choose best image from dpr"""
    optimal_x = np.zeros_like(xstar)
    for i in range(batch_size):
        init_err = min(LA.norm(x[i,:] - xstar[i,:]),LA.norm(x[i,:] + xstar[i,:]))
        ref_err  = min(LA.norm(xneg[i,:] - xstar[i,:]),LA.norm(xneg[i,:] + xstar[i,:]))
        if init_err < ref_err:
            optimal_x[i,:] = x[i,:]
        else:
            optimal_x[i,:] = xneg[i,:]
...
def inv_transform_image_to_uint8(im):
    image = (im.reshape([64,64,3]) + 1.)*(255./2.)
    return image.astype(np.uint8)

def inv_transform_image_mnist(im):
    image = (im.reshape([28,28]) + 1.)/2.
    return image

def resolve_ambiguity(x_hat, xstar, batch_size):
    for i in range(batch_size):
        if LA.norm(x_hat[i,:] + xstar[i,:]) < LA.norm(x_hat[i,:] - xstar[i,:]):
            x_hat[i,:] = -1*x_hat[i,:]
    return x_hat

def print_stats(x_hat, xstar, batch_size):
    x_hat_im = [inv_transform_image_mnist(x_hat[k,:]) for k in range(batch_size)]
    xstar_im = [inv_transform_image_mnist(xstar[ll,:]) for ll in range(batch_size)]
    MSSIM = 0
    loss  = 0
    for t in range(batch_size):
        MSSIM = MSSIM + get_ssim(x_hat_im[t], xstar_im[t])
        loss  = loss + get_l2_loss(x_hat[t,:], xstar[t,:])
    MSSIM = MSSIM / batch_size
    loss  = loss / batch_size
    print "MMSIM:", str(MSSIM)
    print "L2 loss:", str(loss)
    return []
...
def get_opt_reinit_op(opt, var_list, global_step):
    ...
    if isinstance(opt, tf.train.AdamOptimizer):
        b1, b2 = opt.get_beta_accumulators()
        opt_slots.extend( [b1, b2] )
